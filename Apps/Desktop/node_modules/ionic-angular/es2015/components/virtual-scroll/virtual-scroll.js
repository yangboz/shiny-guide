...
export class VirtualScroll {
    ...
    scrollEnd(ev) {
        ...
        this._dom.write(() => {
            ...
            writeToNodes(this._plt, nodes, cells, recordsLength);
            // ******** DOM WRITE ****************
            this._setHeight(estimateHeight(recordsLength, cells[cells.length - 1], this._vHeight, 0.05));
            this._queue = SCROLL_QUEUE_NO_CHANGES;
        });
    }
    /**
    ...
    _listeners() {
        if (!this._scrollSub) {
            ...
            this._scrollSub = this._content.ionScroll.subscribe((ev) => {
                this.scrollUpdate(ev);
            });
            this._scrollEndSub = this._content.ionScrollEnd.subscribe((ev) => {
                this.scrollEnd(ev);
            });
        }
    }
